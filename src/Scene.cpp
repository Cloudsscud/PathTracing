...
void Scene::addShape(const Shape* shape, const glm::vec3& pos, const glm::vec3& scale, const glm::vec3& rotate) {
	...
	m_instances.push_back(ShapeInstance{ shape, world_from_object, glm::inverse(world_from_object) });
}
...
std::optional<HitInfo> Scene::intersect(const Ray& ray, float tmin, float tmax) const{
	...
	for (const auto& instance : m_instances) {
		...
		if (hit_info.has_value()) {
			...
			tmax = hit_info.value().m_hit_t;
			closest_instance = &instance;
		}
	}

	if (closest_instance) {
		closest_hit_info.value().m_hit_pos = closest_instance->m_world_from_object * glm::vec4(closest_hit_info.value().m_hit_pos, 1.f);
		// 法线经过缩放的变换矩阵发生变化 => 原先变换矩阵的逆的转置
		closest_hit_info.value().m_hit_normal = glm::normalize(glm::vec3(glm::transpose(closest_instance->m_object_from_world) * glm::vec4(closest_hit_info.value().m_hit_normal, 0.f)));
	}
